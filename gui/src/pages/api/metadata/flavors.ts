...
import { flavorsDataHandler } from '@/helpers/dataFetching'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Cluster[]>) {
  ...
  const url = getResourcePath({ server: server, kind: 'metadata', resource: 'flavors' })
  ...
  request.get(url, opts, (error, response, body) => {
    ...
    if (response) {
      let machines = flavorsDataHandler(JSON.parse(body).items)
      res.json(machines)
      res.status(200).end()
    }
  })
