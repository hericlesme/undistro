...
func SetupPinnipedCommand(cmd *cobra.Command) (*cobra.Command, GetKubeconfigParams) {
	...
	f.StringVar(&flags.generatedNameSuffix, "generated-name-suffix", "-undistro", "Suffix to append to generated cluster, context, user kubeconfig entries")
...
func newExecConfig(deps KubeconfigDeps, flags GetKubeconfigParams) (*clientcmdapi.ExecConfig, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not determine the undistro executable path: %w", err)
	}
	...
	// If one of the --static-* flags was passed, output a config that runs `undistro login static`.
	...
	// Otherwise continue to parse the OIDC-related flags and output a config that runs `undistro login oidc`.
