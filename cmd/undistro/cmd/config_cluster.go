...
var configClusterClusterCmd = &cobra.Command{
	...
	Long: LongDesc(`
		...
		$HOME/.undistro/undistro.yaml to add new provider or to customize existing ones.

		Each provider configuration links to a repository; undistro uses this information
	...
	Example: Examples(`
		...
		undistro config cluster my-cluster --control-plane-machine-count=3 --worker-machine-count=10

		# Generates a configuration file for creating workload clusters using a template stored in a ConfigMap.
		undistro config cluster my-cluster --from-config-map MyTemplates

		# Generates a configuration file for creating workload clusters using a template from a specific URL.
		undistro config cluster my-cluster --from https://github.com/foo-org/foo-repository/blob/master/cluster-template.yaml

		# Generates a configuration file for creating workload clusters using a template stored locally.
		undistro config cluster my-cluster --from ~/workspace/cluster-template.yaml`),
...
func init() {
	...
	configClusterClusterCmd.Flags().StringVar(&cc.kubernetesVersion, "kubernetes-version", "",
		"The Kubernetes version to use for the workload cluster. If unspecified, the value from OS environment variables or the .undistro/undistro.yaml config file will be used.")
