...
func (c *undistroClient) Delete(options DeleteOptions) error {
	...
	var providersToDelete []undistrov1.Provider

	if options.DeleteAll {
		providersToDelete = installedProviders.Items
	} else {
		// Otherwise we are deleting only a subset of providers.
		var providers []undistrov1.Provider
		providers = appendProviders(providers, undistrov1.CoreProviderType, options.CoreProvider)
		providers = appendProviders(providers, undistrov1.BootstrapProviderType, options.BootstrapProviders...)
		providers = appendProviders(providers, undistrov1.ControlPlaneProviderType, options.ControlPlaneProviders...)
		providers = appendProviders(providers, undistrov1.InfrastructureProviderType, options.InfrastructureProviders...)
...
func appendProviders(list []undistrov1.Provider, providerType undistrov1.ProviderType, names ...string) []undistrov1.Provider {
	for _, name := range names {
		if name == "" {
			continue
		}

		list = append(list, undistrov1.Provider{
