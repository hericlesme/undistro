...
func Test_undistroClient_InitImages(t *testing.T) {
	type field struct {
		client *fakeClient
	}
...
func Test_undistroClient_Init(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			field: field{
				client: fakeEmptyCluster(), // undistro client for an empty management cluster (with repository setup for capi, bootstrap, control plane and infra provider)
				hasCRD: false,
			},
			args: args{
				coreProvider:           "",  // with an empty cluster, a core provider should be added automatically
		...
		},
		{
			...
			field: field{
				client: fakeInitializedCluster(), // undistro client for an management cluster with capi installed (with repository setup for capi, bootstrap, control plane and infra provider)
				hasCRD: true,
			},
			args: args{
...
// undistro client for an empty management cluster (with repository setup for capi, bootstrap and infra provider)
...
func fakeRepositories(config *fakeConfigClient, providers []Provider) []*fakeRepositoryClient {
	repository1 := newFakeRepository(capiProviderConfig, config).
		...
		WithMetadata("v1.1.0", &undistrov1.Metadata{
			ReleaseSeries: []undistrov1.ReleaseSeries{
	...
	repository2 := newFakeRepository(bootstrapProviderConfig, config).
		...
		WithMetadata("v2.1.0", &undistrov1.Metadata{
			ReleaseSeries: []undistrov1.ReleaseSeries{
...
// undistro client for a management cluster with capi installed (with repository setup for capi, bootstrap and infra provider)
