...
// It is important to notice that undistro applies a set of processing steps to the “raw” component YAML read
...
type Components interface {
	...
	// InventoryObject returns the undistro inventory object representing the provider that will be
...
// undistro library. These are user specified inputs.
...
func NewComponents(input ComponentsInput) (*components, error) {
	...
	// undistro does such modification for the user, and in order to do so, it is required to split objects in two sets;
	...
	// Add an identifying label to shared components so next invocation of init, undistro delete and undistro upgrade can act accordingly.
...
// fixSharedLabels ensures all the shared components have an identifying label so next invocation of init, undistro delete
// and undistro upgrade can act accordingly.
