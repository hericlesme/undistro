...
func Test_providerUpgrader_getUpgradeInfo(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				reader: test.NewFakeReader().
					WithProvider("p1", undistrov1.InfrastructureProviderType, "https://somewhere.com"),
			...
			args: args{
				provider: fakeProvider("p1", undistrov1.InfrastructureProviderType, "v1.0.1", "p1-system", ""),
			},
			want: &upgradeInfo{
		...
		},
		{
			...
			args: args{
				provider: fakeProvider("p1", undistrov1.InfrastructureProviderType, "v1.0.0", "p1-system", ""),
			},
			want:    nil,
			wantErr: true,
		},
		...
		},
		{
			...
			fields: fields{
				...
				repository: test.NewFakeRepository(). //without metadata
									...
									WithMetadata("v1.1.1", &undistrov1.Metadata{
						ReleaseSeries: []undistrov1.ReleaseSeries{
							{Major: 1, Minor: 0, Contract: "v1alpha3"},
							// missing 1.1 series
						},
...
func Test_upgradeInfo_getContractsForUpgrade(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			field: field{
				metadata: &undistrov1.Metadata{ // metadata defining more release series, all linked to a single contract
					ReleaseSeries: []undistrov1.ReleaseSeries{
		...
		},
		{
			...
			field: field{
				metadata: &undistrov1.Metadata{ // metadata defining more release series, linked to different contracts
					ReleaseSeries: []undistrov1.ReleaseSeries{
						{Major: 0, Minor: 1, Contract: "v1alpha3"},
