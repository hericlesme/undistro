...
func shouldInstallSharedComponents(providerList *undistrov1.ProviderList, provider undistrov1.Provider) (bool, error) {
...
func (i *providerInstaller) Validate() error {
	...
	for _, components := range i.installQueue {
		...
		if providerContract != managementGroupContract && provider.GetProviderType() != undistrov1.UndistroProviderType {
...
func (i *providerInstaller) getProviderContract(providerInstanceContracts map[string]string, provider undistrov1.Provider) (string, error) {
...
func simulateInstall(providerList *undistrov1.ProviderList, components repository.Components) (*undistrov1.ProviderList, error) {
	provider := components.InventoryObject()
