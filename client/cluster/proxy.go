...
func (k *proxy) GetConfig() (*rest.Config, error) {
	...
	if err != nil {
		if strings.HasPrefix(err.Error(), "invalid configuration:") {
			return nil, errors.New(strings.Replace(err.Error(), "invalid configuration:", "invalid kubeconfig file; undistro requires a valid kubeconfig file to connect to the management cluster:", 1))
		}
		return nil, err
	}
	restConfig.UserAgent = fmt.Sprintf("undistro/%s (%s)", version.Get().GitVersion, version.Get().Platform)
