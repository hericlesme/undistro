...
import (
	...
	"github.com/getupio-undistro/undistro/config"
...
const (
	embeddedCustomResourceDefinitionPath = "config/manifest/undistro-api.yaml"
...
type InventoryClient interface {
	...
	GetDefaultProviderName(providerType undistrov1.ProviderType) (string, error)
	...
	GetDefaultProviderVersion(provider string, providerType undistrov1.ProviderType) (string, error)
	...
	GetDefaultProviderNamespace(provider string, providerType undistrov1.ProviderType) (string, error)
...
func (p *inventoryClient) EnsureCustomResourceDefinitions() error {
	...
	// Being this the first connection of many undistro operations, we want to fail fast if there is no
	...
	log.V(1).Info("Installing the undistro inventory CRD")
	...
	if err != nil {
		return errors.Wrap(err, "failed to parse yaml for undistro inventory CRDs")
	}
...
func checkInventoryCRDs(proxy Proxy) (bool, error) {
	...
	l := &undistrov1.ProviderList{}
	if err = c.List(ctx, l); err == nil {
		return true, nil
	}
	if !apimeta.IsNoMatchError(err) {
		return false, errors.Wrap(err, "failed to check if the undistro inventory CRD exists")
	}
	return false, nil
}
...
func (p *inventoryClient) createObj(o unstructured.Unstructured) error {
	...
	if err := c.Create(ctx, &o); err != nil {
		...
		return errors.Wrapf(err, "failed to create undistro inventory CRDs component: %s, %s/%s", o.GroupVersionKind(), o.GetNamespace(), o.GetName())
	}
	return nil
}

func (p *inventoryClient) Create(m undistrov1.Provider) error {
	// Create the Kubernetes object.
	createInventoryObjectBackoff := newWriteBackoff()
	return retryWithExponentialBackoff(createInventoryObjectBackoff, func() error {
		...
		currentProvider := &undistrov1.Provider{}
...
func listProviders(proxy Proxy, providerList *undistrov1.ProviderList) error {
...
func (p *inventoryClient) GetDefaultProviderName(providerType undistrov1.ProviderType) (string, error) {
...
func (p *inventoryClient) GetDefaultProviderVersion(provider string, providerType undistrov1.ProviderType) (string, error) {
	providerList, err := p.List()
	if err != nil {
		return "", err
	}
...
func (p *inventoryClient) GetDefaultProviderNamespace(provider string, providerType undistrov1.ProviderType) (string, error) {
	providerList, err := p.List()
	if err != nil {
		return "", err
	}
