	...
	for _, c := range f.Repositories {
		...
		go func(r *repo.ChartRepository) {
			if _, err := r.DownloadIndexFile(); err != nil {
				log.Log.Error(err, "unable to get an update from the chart repository", "url", r.Config.URL)
			}
			wg.Done()
		}(r)
	}
...
func (h *HelmV3) RepositoryImport(path string) error {
	...
	for _, c := range s.Repositories {
		if t.Has(c.Name) {
			log.Log.Error(err, "repository with name already exists", "name", c.Name, "url", c.URL)
			continue
		}
		r, err := newChartRepository(c)
		if err != nil {
			log.Log.Error(err, "name", c.Name, "url", c.URL)
			continue
		}
		if _, err := r.DownloadIndexFile(); err != nil {
			log.Log.Error(err, "name", c.Name, "url", c.URL)
		...
		t.Add(c)
		log.Log.Info("successfully imported repository", "name", c.Name, "url", c.URL)
	}
