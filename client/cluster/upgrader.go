...
type ProviderUpgrader interface {
	...
	// ApplyPlan executes an upgrade following an UpgradePlan generated by undistro.
	ApplyPlan(coreProvider undistrov1.Provider, clusterAPIVersion string) error

	// ApplyCustomPlan plan executes an upgrade using the UpgradeItems provided by the user.
	ApplyCustomPlan(coreProvider undistrov1.Provider, providersToUpgrade ...UpgradeItem) error
}
...
func (u *providerUpgrader) ApplyPlan(coreProvider undistrov1.Provider, contract string) error {
	log := logf.Log
...
func (u *providerUpgrader) ApplyCustomPlan(coreProvider undistrov1.Provider, upgradeItems ...UpgradeItem) error {
	log := logf.Log
	log.Info("Performing upgrade...")
...
func (u *providerUpgrader) getManagementGroup(coreProvider undistrov1.Provider) (*ManagementGroup, error) {
	managementGroups, err := u.providerInventory.GetManagementGroups()
	if err != nil {
		return nil, err
	}
...
func (u *providerUpgrader) createCustomPlan(coreProvider undistrov1.Provider, upgradeItems []UpgradeItem) (*UpgradePlan, error) {
	// Retrieves the management group.
	managementGroup, err := u.getManagementGroup(coreProvider)
	if err != nil {
		return nil, err
...
func (u *providerUpgrader) getProviderContractByVersion(provider undistrov1.Provider, targetVersion string) (string, error) {
