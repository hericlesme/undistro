...
var _ = Describe("Create EC2 cluster 1.22", func() {
	...
	It("Should generate recommend cluster spec 1.22", func() {
		_, _, err := undcli.Create("cluster", "ec2-22-e2e", "-n", "e2e", "--infra", "aws", "--flavor", "ec2", "--ssh-key-name", "undistro", "--generate-file")
	...
	It("Should create EC2 cluster 1.22", func() {
		sout, _, err := undcli.Apply("-f", "../../testdata/ec2-22.yaml")
		fmt.Println(err)
		Expect(err).ToNot(HaveOccurred())
		fmt.Println(sout)
		...
		sout, _, err = undcli.Get("kubeconfig", "ec2-22-e2e", "-n", "e2e", "--admin")
		Expect(err).ToNot(HaveOccurred())
		...
		sout, _, err = undcli.Delete("-f", "../../testdata/ec2-22.yaml")
		Expect(err).ToNot(HaveOccurred())
		fmt.Println(sout)
