/*
Copyright 2020-2022 The UnDistro authors
...
var _ = Describe("Create EC2 cluster 1.22", func() {
	var (
		clusterClient client.Client
	)
	It("Should generate recommend cluster spec 1.22", func() {
		cmd := exec.NewCommand(
			exec.WithCommand("undistro"),
			exec.WithArgs("create", "cluster", "ec2-22-e2e", "-n", "e2e", "--infra", "aws", "--flavor", "ec2", "--ssh-key-name", "undistro", "--generate-file"),
		)
		_, _, err := cmd.Run(context.Background())
		Expect(err).ToNot(HaveOccurred())
		_, err = os.Stat("ec2-22-e2e.yaml")
		Expect(err).ToNot(HaveOccurred())

	})
	It("Should create EC2 cluster 1.22", func() {
		cmd := exec.NewCommand(
			exec.WithCommand("undistro"),
			exec.WithArgs("apply", "-f", "./testdata/ec2-22.yaml"),
		)
		...
		cmd = exec.NewCommand(
			exec.WithCommand("undistro"),
			exec.WithArgs("get", "kubeconfig", "ec2-22-e2e", "-n", "e2e", "--admin"),
		)
		out, _, err = cmd.Run(context.Background())
		Expect(err).ToNot(HaveOccurred())
		...
		cmd = exec.NewCommand(
			exec.WithCommand("undistro"),
			exec.WithArgs("delete", "-f", "./testdata/ec2-22.yaml"),
		)
		out, _, err = cmd.Run(context.Background())
		Expect(err).ToNot(HaveOccurred())
		fmt.Println(string(out))
