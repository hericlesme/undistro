...
var _ = Describe("Create VM cluster 1.21", func() {
	var (
	...
	It("Should generate recommend cluster spec 1.21", func() {
		cmd := exec.NewCommand(
			...
			exec.WithArgs("create", "cluster", "vm-21-e2e", "-n", "e2e", "--infra", "azure", "--flavor", "vm", "--ssh-key-name", "undistro", "--generate-file"),
		)
		_, _, err := cmd.Run(context.Background())
		Expect(err).ToNot(HaveOccurred())
		_, err = os.Stat("vm-21-e2e.yaml")
		Expect(err).ToNot(HaveOccurred())

	})
	It("Should create VM cluster 1.21", func() {
		cmd := exec.NewCommand(
			exec.WithCommand("undistro"),
			exec.WithArgs("apply", "-f", "./testdata/vm-21.yaml"),
		)
		...
		cmd = exec.NewCommand(
			exec.WithCommand("undistro"),
			exec.WithArgs("get", "kubeconfig", "vm-21-e2e", "-n", "e2e", "--admin"),
		)
		out, _, err = cmd.Run(context.Background())
		Expect(err).ToNot(HaveOccurred())
		...
		cmd = exec.NewCommand(
			exec.WithCommand("undistro"),
			exec.WithArgs("delete", "-f", "./testdata/vm-21.yaml"),
		)
		out, _, err = cmd.Run(context.Background())
		Expect(err).ToNot(HaveOccurred())
		fmt.Println(string(out))
