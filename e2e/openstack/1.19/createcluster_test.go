...
import (
	...
	undistro "github.com/getupio-undistro/clilib"
	"github.com/getupio-undistro/meta"
	appv1alpha1 "github.com/getupio-undistro/undistro/apis/app/v1alpha1"
	testcases "github.com/getupio-undistro/undistro/e2e/openstack"
...
const (
	clusterName = "openstack-k8s"
	clusterNamespace = "e2e"
	infrastructureProvider = "openstack"
	// In Cluster API Provider OpenStack we just has one way of create clusters
	infrastructureFlavor = "openstack"
	sshKeyName = "undistro"
)

var _ = Describe(testcases.CreateClusterDescription, func() {
	var (
		clusterClient client.Client
		undistroClient = undistro.NewCLI(GinkgoWriter)
	)
	// May we can get these variable values from environment

	createCmdArgs := []string{
		undistro.Create, clusterName,
		"--namespace", clusterNamespace,
		"--infra", infrastructureProvider,
		"--flavor", infrastructureFlavor,
		"--ssh-key-name", sshKeyName,
	}

	It("Should generate cluster specification pointing to 1.19 Kubernetes Version", func() {
		_, _, err := undistroClient.Create(createCmdArgs...)
